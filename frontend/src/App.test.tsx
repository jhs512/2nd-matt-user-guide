...
test('pending save prevents a new draft and failed save preserves input', async () => {
 ...
 const fetcher = vi.fn().mockImplementation((url: string, init?: RequestInit) => {
  if (url.endsWith('/api/auth/login')) return Promise.resolve(new Response(JSON.stringify({ accessToken: 'test-token' })))
  if (init?.method === 'POST') return new Promise((_resolve, reject) => { failSave = reject })
  return Promise.resolve(new Response(JSON.stringify({ items: [], totalPages: 0, totalElements: 0 })))
 })
 vi.stubGlobal('fetch', fetcher)
 render(<App />)
 ...
 fireEvent.change(screen.getByLabelText('제목'), { target: { value: '보존할 제목' } })
 ...
 expect(await screen.findByRole('alert')).toHaveTextContent('일시적인 연결 실패')
